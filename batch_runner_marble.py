...
def process_images():
    ...
    print(f"⚙️  STRENGTH={STRENGTH}, MAX_STRENGTH={MAX_STRENGTH}")
    print("=" * 60)
    ...
    for idx, img_path in enumerate(image_files, 1):
        ...
        config_cmd = (
            ...
            f"--model \"{MODEL_PATH}\" "
            f"--strength {STRENGTH} "
            f"--max-strength {MAX_STRENGTH} "  # ✅ 传递 max_strength
            f"--cfg {CFG} "
            f"--steps {STEPS}"
        )
        ...
        if not config_files:
            print(f"❌ 没有找到 marble_batch_config_*.json")
            continue
