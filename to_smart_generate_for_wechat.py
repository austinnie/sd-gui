...
STEPS = 20 
# 👇 【核心边长控制】最大输出边长（像素）
# 512 = 速度快，适合半身/头像/特写（面部清晰）
# 768 = 速度适中，适合全身照/大场景（面部不会崩）
# 1024 = 速度较慢，适合极度讲究细节的风景和雕塑
MAX_LIMIT = 768 
...
# 👇 选择你要使用的模型 (填 1, 2, 或 3)
# 根据你截图里的模型，我帮你预设了 3 个最适合 CPU 跑、适合发公众号的模型
ACTIVE_MODEL = 1  

# 模型配置清单
SD_MODEL_PATH_1 = "../models/sd-v1-5/anytimeRealistic_v10.safetensors"      # 写实/唯美 (日常首选)
SD_MODEL_PATH_2 = "../models/sd-v1-5/henmixreal_v10_henmixrealV10.safetensors" # 亚洲唯美/古风
SD_MODEL_PATH_3 = "../models/sd-v1-5/sd-v1-5-tiny.safetensors"              # 极速发图 (速度最快)
...
def build_prompts():
    ...
    for category in ["daily", "ancient", "gallery"]:
        ...
        # 根据 TARGET_TYPE 获取主体描述（扩展版）
        if target == "女性人物":
            subjects = [
                # 版本 A：双手自然放在腿上（避免手部变形的最优选择）
        ...
        # 生成 GEN_COUNT 组不同的提示词
        for i in range(GEN_COUNT):
            # 随机取不同的元素组合，确保每一张都不一样
...
def generate_style(pipe, init_image, prompt, output_filename, strength=0.45):
    ...
    # ===== 核心提速：强制限制边长在 512 以内 =====
    # 512是手机壁纸和公众号插图的最佳平衡点，速度比1024快4倍！
    # ===== 读取全局配置的边长限制 =====
    max_limit = MAX_LIMIT 
...
def main():
    ...
    output_root = os.path.join("./output/wechat", f"{TARGET_TYPE}_批量_{timestamp}")
