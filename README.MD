...
v0 （此前有半年左右，一直都是本地不断的修改，从0开始，没有用git管理）
从0搭建环境
下载模型
模型CPU上跑通模型
提示词生成图片
图片生成图片
从v0一直到v8_universe_generator，功能稳定后才开始从git管理，继续扩展功能
请优化，把通用部分作为utils，并全部改成config来加载，更大的灵活性
经过了多次重构，把独立的功能，变成了可以复用的services
